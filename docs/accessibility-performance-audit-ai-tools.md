...
| Screen-reader usability of calculator results | **Fixed in this pass.** None of the 10 calculator components previously marked their result region as `aria-live`, so a screen-reader user changing an input had no way to know the result updated without re-navigating to it. Added `aria-live="polite"` to each calculator's result panel (`ReadinessAssessment` already had it). |
